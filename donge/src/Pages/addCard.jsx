import axios from "axios";
import Header from "../Componnets/Header";
import { Navigate, useNavigate } from "react-router-dom";
import { useState } from "react";

export default function AddCard() {
  const navigate = useNavigate();

  const value = localStorage.getItem("token");
  const [carderror, setcarderror] = useState("");

  async function addCardInfo(e) {
    e.preventDefault();

    const formData = new FormData(e.target);
    const data = Object.fromEntries(formData.entries());
    try {
      const resp = await axios.post(
        "https://dongapi.phaedra.ir/CORE/cards/add",
        data,
        {
          headers: {
            Authorization: `Bearer ${JSON.parse(value)}`,
          },
        }
      );
      if (resp.data.success == true) navigate("/profile");
    } catch (error) {
      if (!error.response.data.success)
        setcarderror(error.response.data.message);
    }
    setTimeout(() => setcarderror(""), 5000);
  }

  return (
    <>
      <div>
        <Header enableMenu={false} currentMenu={0} />
        <div className="font-iranyekan font-medium ">
          {/* Sub Header */}
          <div className="bg-subHeaderColor flex justify-between items-center mt-16 h-10 px-10">
            <a href="/Profile">
              <svg
                width="35"
                height="35"
                viewBox="0 0 48 46"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                xmlnsXlink="http://www.w3.org/1999/xlink"
              >
                <rect width="48" height="46" fill="url(#pattern0_79_188)" />
                <defs>
                  <pattern
                    id="pattern0_79_188"
                    patternContentUnits="objectBoundingBox"
                    width="1"
                    height="1"
                  >
                    <use
                      xlinkHref="#image0_79_188"
                      transform="matrix(0.00641026 0 0 0.00668896 0 -0.0217391)"
                    />
                  </pattern>
                  <image
                    id="image0_79_188"
                    width="156"
                    height="156"
                    xlinkHref="data:image/png;base64,"
                  />
                </defs>
              </svg>
            </a>
            <p>ثبت کارت جدید</p>
          </div>
          <div className="flex flex-col justify-center items-center mt-10">
            <form
              onSubmit={addCardInfo}
              className="flex flex-col justify-center items-center gap-3 p-5 rounded-2xl border border-slate-200 drop-shadow shadow-xl"
            >
              {/* Card Name */}
              <div className="flex flex-col justify-center items-end">
                <label htmlFor="cardName" className="mb-2 font-medium text-xl">
                  نام کارت
                </label>
                <input
                  className="bg-textBoxColor rounded-[15px] border-2 placeholder:text-gray-400 text-black border-backPurple p-3 mb-4"
                  dir="rtl"
                  id="cardName"
                  name="cardName"
                  placeholder="بلو بانک"
                  type="text"
                />
                {/* Card Number */}
                <label htmlFor="cardPan" className="mb-2 font-medium text-xl">
                  شماره کارت
                </label>
                <input
                  className="bg-textBoxColor rounded-[15px] border-2 placeholder:text-gray-400 text-black border-backPurple p-3"
                  dir="ltr"
                  id="cardPan"
                  name="cardPan"
                  placeholder="6219********7707"
                  type="text"
                />
              </div>
              <button
                type="submit"
                className="bg-buttonColor text-white text-center w-44 py-2 rounded-lg hover:scale-110 transition-all duration-200 mt-5"
              >
                ثبت کارت جدید
              </button>
              <p>{carderror}</p>
            </form>
          </div>
        </div>
      </div>
    </>
  );
}
